...
CREATE POLICY "patients_all_own_practice" ON public.patients FOR ALL TO authenticated
...
CREATE POLICY "appointments_all_own_practice" ON public.appointments FOR ALL TO authenticated
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()));

-- invoices table
DROP POLICY IF EXISTS "invoices_all_own_practice" ON public.invoices;
CREATE POLICY "invoices_all_own_practice" ON public.invoices FOR ALL TO authenticated
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()));

-- invoice_items table
DROP POLICY IF EXISTS "invoice_items_all_own" ON public.invoice_items;
CREATE POLICY "invoice_items_all_own" ON public.invoice_items FOR ALL TO authenticated
...
CREATE POLICY "payments_all_own" ON public.payments FOR ALL TO authenticated
  USING (invoice_id IN (SELECT i.id FROM invoices i JOIN practice_profiles pp ON pp.id = i.practice_profile_id WHERE pp.user_id = auth.uid()))
  WITH CHECK (invoice_id IN (SELECT i.id FROM invoices i JOIN practice_profiles pp ON pp.id = i.practice_profile_id WHERE pp.user_id = auth.uid()));

-- service_records table
DROP POLICY IF EXISTS "service_records_all_own_practice" ON public.service_records;
CREATE POLICY "service_records_all_own_practice" ON public.service_records FOR ALL TO authenticated
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()));

-- tasks table
DROP POLICY IF EXISTS "tasks_all_own_practice" ON public.tasks;
CREATE POLICY "tasks_all_own_practice" ON public.tasks FOR ALL TO authenticated
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()));

-- reminders table
DROP POLICY IF EXISTS "reminders_all_own_practice" ON public.reminders;
CREATE POLICY "reminders_all_own_practice" ON public.reminders FOR ALL TO authenticated
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = auth.uid()));

-- patient_consents table
DROP POLICY IF EXISTS "patient_consents_all_own" ON public.patient_consents;
CREATE POLICY "patient_consents_all_own" ON public.patient_consents FOR ALL TO authenticated
