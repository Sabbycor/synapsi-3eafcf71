...
-- Fix CRITICAL issue: all RLS policies are RESTRICTIVE (they cannot grant access on their own).
-- Drop each RESTRICTIVE policy and recreate as PERMISSIVE (default).
...
CREATE POLICY "appointments_all_own_practice" ON public.appointments
  ...
  USING (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = (SELECT auth.uid())))
  WITH CHECK (practice_profile_id IN (SELECT id FROM practice_profiles WHERE user_id = (SELECT auth.uid())));

-- audit_logs
DROP POLICY IF EXISTS "audit_logs_insert_own" ON public.audit_logs;
CREATE POLICY "audit_logs_insert_own" ON public.audit_logs
  FOR INSERT TO authenticated
  WITH CHECK ((actor_user_id = (SELECT auth.uid())) OR (actor_user_id IN (SELECT id FROM users WHERE id = (SELECT auth.uid()))));

DROP POLICY IF EXISTS "audit_logs_select_own" ON public.audit_logs;
CREATE POLICY "audit_logs_select_own" ON public.audit_logs
  FOR SELECT TO authenticated
  USING ((actor_user_id = (SELECT auth.uid())) OR (actor_user_id IN (SELECT id FROM users WHERE id = (SELECT auth.uid()))));
...
CREATE POLICY "invoice_items_all_own" ON public.invoice_items
  FOR ALL TO authenticated
  USING (invoice_id IN (SELECT i.id FROM invoices i JOIN practice_profiles pp ON pp.id = i.practice_profile_id WHERE pp.user_id = (SELECT auth.uid())))
  WITH CHECK (invoice_id IN (SELECT i.id FROM invoices i JOIN practice_profiles pp ON pp.id = i.practice_profile_id WHERE pp.user_id = (SELECT auth.uid())));
...
CREATE POLICY "patient_consents_all_own" ON public.patient_consents
  FOR ALL TO authenticated
  USING (patient_id IN (SELECT p.id FROM patients p JOIN practice_profiles pp ON pp.id = p.practice_profile_id WHERE pp.user_id = (SELECT auth.uid())))
  WITH CHECK (patient_id IN (SELECT p.id FROM patients p JOIN practice_profiles pp ON pp.id = p.practice_profile_id WHERE pp.user_id = (SELECT auth.uid())));
...
-- Also ensure the auth trigger exists (was missing per db-triggers inspection)
